...
use backhopper_core::model::names::{CommitSha, ProjectName};
...
pub fn handle(args: &GlobalArgs, cmd: BisectCmd) -> CliResult<CommandOutcome> {
    ...
    match cmd {
        BisectCmd::Commit {
            ...
            let commit = expand_prefix_enriched(&repo, &commit, &repo_dir_path)?;
...
fn run_commit(
    ...
    for tag in &tags {
        let evaluate = || -> CliResult<SeriesEvaluation> {
            ...
            let scope = check::build_pin_scope(p, &snap);
    ...
    let exit = CommandOutcome::from_success(!any_blocking);

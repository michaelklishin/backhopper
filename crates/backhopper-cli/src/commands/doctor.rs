...
pub struct Totals {
    pub series: usize,
    pub pins: usize,
    /// Pins whose snapshot is on disk, current, stale, or unversioned alike.
    pub present: usize,
    pub missing: usize,
    pub stale_extractor: usize,
    /// Present but carrying no `extractor-version` header: a
    /// pre-0.31.0 snapshot, counted separately from `stale_extractor`
    /// because there is no recorded version to compare against
    /// `expected`, only a `format_version` to report
    pub unversioned_extractor: usize,
}
...
impl Totals {
    pub fn record(&mut self, status: &SnapshotStatus) {
        ...
        match status {
            ...
            SnapshotStatus::Unversioned { .. } => {
                self.present += 1;
                self.unversioned_extractor += 1;
            }
...
/// extractor, on disk with no recorded extractor version at all, or
/// absent. `Unversioned` is maximally stale: a snapshot old enough to
/// carry no extractor version is the most suspect file in the store,
/// not the healthiest.
...
// Success when every pin is present and current; PartialSuccess when any is missing, stale, or unversioned
pub fn doctor_exit_code(totals: &Totals) -> CommandOutcome {
    CommandOutcome::from_success(
        !(totals.missing > 0 || totals.stale_extractor > 0 || totals.unversioned_extractor > 0),
    )
}
...
/// A snapshot with no recorded extractor version is not treated as
/// current: it predates versioning and is the most suspect file in
/// the store, not the healthiest.
fn snapshot_status(
    cfg: &Config,
    store: &SnapshotStore<ReadOnly>,
...
/// The remedy phrasing for a stale or unversioned snapshot, sharing
/// `snapshot_refresh_command` with `snapshots generate --refresh-stale`
/// so the phrasing cannot drift.
fn extractor_stale_note(status: &SnapshotStatus, resolved: Option<&Pin>) -> Option<String> {
    let pin = resolved?;
    match status {
        SnapshotStatus::Stale { stored, expected } => Some(format!(
            "extractor stored={stored} expected={expected}: run {}",
            snapshot_refresh_command(&pin.project, &pin.tag)
        )),
        SnapshotStatus::Unversioned {
            format_version,
            expected,
        } => Some(format!(
            "no extractor version recorded (format-version {format_version}, expected {expected}): run {}",
            snapshot_refresh_command(&pin.project, &pin.tag)
        )),
        SnapshotStatus::Current | SnapshotStatus::Missing => None,
    }
}
...
fn render_text(w: &mut dyn Write, payload: &DoctorPayload, style: TableStyle) -> CliResult<()> {
    ...
    if payload.totals.unversioned_extractor > 0 {
        summary.push_str(&format!(
            ", {} unversioned-extractor",
            payload.totals.unversioned_extractor
        ));
    }
    writeln!(w, "{summary}")?;
    writeln!(
    ...
    if payload.totals.missing > 0
        || payload.totals.stale_extractor > 0
        || payload.totals.unversioned_extractor > 0
    {
        writeln!(w)?;
        writeln!(
            w,
...
pub fn snapshot_cell(status: &SnapshotStatus) -> &'static str {
    match status {
        ...
        SnapshotStatus::Unversioned { .. } => "UNVERSIONED",

...
//!
//! Reads `.ex` and `.exs` source and yields the `Module` records from
//! `backhopper-core`. Recognized:
//!  * `defmodule X.Y do ... end`         module names; nested modules become separate entries
//!  * `def name(args)` and `def name(args) do` exported functions
...
//!  * `@moduledoc false` and `@doc false` visibility hints
